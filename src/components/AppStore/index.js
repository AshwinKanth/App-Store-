...
class AppStore extends Component {
  state = {searchInput: '', onActiveTabId: tabsList[0].tabId}

  getFilteredAppsList = searchedApps => {
    const {onActiveTabId} = this.state

    const filteredApps = searchedApps.filter(
      eachApp => eachApp.category === onActiveTabId,
    )
    return filteredApps
  }

  onTabChange = tabId => {
    this.setState({onActiveTabId: tabId})
  }
  ...
  render() {
    const {searchInput, onActiveTabId} = this.state
          ...
          <ul className="tabs-container">
            {tabsList.map(eachTab => (
              <TabItem
                ...
                isActive={onActiveTabId === eachTab.tabId}
              />
            ))}
          </ul>
